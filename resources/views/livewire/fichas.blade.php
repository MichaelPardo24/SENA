            ...
            <table class="table-auto w-full">
                ...
                <tbody class="text-sm divide-y divide-gray-100">
                    @foreach ($fichas as $ficha)
                        <tr>
                            <td class="p-2 whitespace-nowrap text-left font-bold text-gray-800">{{ $ficha->code}}</td>
                            <td class="p-2 whitespace-nowrap text-left font-semibold text-gray-700">{{ Str::limit($ficha->program->name, 50)}}</td>
                            <td class="p-2 whitespace-nowrap text-left font-medium text-gray-700">{{ $ficha->start_school_stage->format('M-d-Y')}}</td>
                            <td class="p-2 whitespace-nowrap text-left font-medium text-gray-700">{{ $ficha->end_school_stage->format('M-d-Y')}}</td>
                            <td class="p-2 whitespace-nowrap text-left font-medium text-gray-700">{{ $ficha->start_production_stage->format('M-d-Y')}}</td>
                            <td class="p-2 whitespace-nowrap text-left font-medium text-gray-700">{{ $ficha->end_production_stage->format('M-d-Y')}}</td>
                            <td class="p-2 whitespace-nowrap">
                                <div class="flex">
                                    <a class="px-1 flex-auto" href="{{ route('fichas.show', $ficha) }}"><i class="fa-solid fa-eye fa-xl" style="color:gray"></i></a>

                                    <a class="px-1 flex-auto" href="{{ route('fichas.edit', $ficha) }}"><i class="fa-solid fa-user-pen fa-xl" style="color:gray"></i></a>

                                    <button class="px-1 flex-auto" data-modal-toggle="popup-modal" data-id="{{ $ficha->code }}" ><i class="fa-solid fa-trash fa-xl" style="color:red"></i></button>
                                        ...
                                        <div class="relative px-4 w-full max-w-md h-full md:h-auto">
                                            ...
                                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                ...
                                                <div class="p-6 pt-0 text-center">
                                                    ...
                                                    <form action="{{ route('fichas.destroy', $ficha) }}" method="POST" class="m-3">
                                                        @method('DELETE')
                                                        @csrf
