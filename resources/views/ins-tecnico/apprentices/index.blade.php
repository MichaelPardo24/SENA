<x-app-layout>
    ...
    <div class="py-10">
        <div class="mx-auto max-w-4xl bg-white shadow-lg rounded-sm border border-gray-200">
            @if ($ficha->trashed())
                {{-- Aviso si el instructor quere modificar alumnos en una ficha ya archivada --}}
                ...
                <div class="flex items-center bg-sky-600 text-white text-sm font-bold px-4 py-3" role="alert">
                    ...
                    <p>Esta ficha esta archivada, proceda con cuidado.</p>
                </div>    
            @endif

            {{-- table --}}
            @livewire('ins-tecnico.fichas.apprentices', ['ficha' => $ficha])
        </div>
    </div>
</x-app-layout>