                ...
                <div class="p-3 bg-white rounded shadow-lg">
                    ...
                    <form action="{{ route('files.update', $file) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="flex flex-row p-2 mt-4 justify-around">
                            ...
                            <a href="{{route('users.files.index', auth()->user())}}" class="font-semibold uppercase tracking-widest rounded bg-slate-700 text-xs text-slate-200 px-4 py-2 transition-all duration-300 hover:bg-slate-900">Index</a>
                        </div>
                    </form>
                </div>
                <div class="mt-2 bg-red-200 p-3 rounded shadow-lg">
                    <form action="{{ route('files.destroy', $file) }}" class="w-full" method="POST">
                        @method('DELETE')
                        @csrf
