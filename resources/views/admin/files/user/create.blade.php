            ...
            <div class="bg-white p-4 overflow-hidden sm:max-w-md sm:mx-auto  shadow-xl sm:rounded-lg">
                ...
                <form action="{{ route('users.files.store', $user) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="flex flex-row p-2 mt-4 justify-around">
                        ...
                        <a href="{{route('users.files.index', auth()->user())}}" class="font-semibold uppercase tracking-widest rounded bg-slate-700 text-xs text-slate-200 px-4 py-2 transition-all duration-300 hover:bg-slate-900">Regresar</a>
                    </div>
                </form>
            </div>
        </div>
